...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Andrew Soltesz" };
...
//Helicopter rotation rate
const int RotationRate = 30;
...
float	Time;					// elapsed time in animation cycle
int		ViewMode;				// view to use
...
Display( )
{
	...
	if (ViewMode == OUTSIDE) {
		...
		gluLookAt(-0.4, 1.8, -4.9, -0.4, 1.8, -6., 0., 1., 0.);
	}
	...
	//draw the main helicopter rotor:

	glPushMatrix();
		glTranslatef(0., 2.9, -2.);
		glRotatef(360 * Time * RotationRate, 0., 1., 0.);
		glRotatef(90., 1., 0., 0.);
		glScalef(5., 1., 1.);
	...
	glPushMatrix();
		...
		glRotatef(360 * Time * 3 * RotationRate, 1., 0., 0.);
		glRotatef(90., 0., 1., 0.);
		glScalef(1.5, 1., 1.);
		glColor4f(0.6, 0.6, 0.6, 0.6);
		glCallList(BladeList);
...
InitLists( )
{
	...
	glNewList( HeliList, GL_COMPILE );
		...
		for (i = 0, tp = Helitris; i < Helintris; i++, tp++)
		{
			...
			if (n[1] > 1.)
				n[1] = 1.;
			glColor3f(n[1] * 0.247, n[1] * 0.298, n[1] * 0.420);
